...
if __name__ == "__main__":
    ...
    for index, row in imu1.iterrows():
        if not row.isnull().any():
            ...
            speed_prev = speed_list1[-1] if len(speed_list1) > 1 else 0
            speed_update1 = speed_list1.append(speed_update(row['AccX(mg)'], row['AccY(mg)'], row['AccZ(mg)'], ax_list1[-1], ay_list1[-1], az_list1[-1], imu1_positions, DELTA_T, imu1_initial_position))
    ...
    for index, row in imu2.iterrows():
        if not row.isnull().any():
            ...
            speed_prev = speed_list2[-1] if len(speed_list2) > 1 else 0
            speed_update2 = speed_list2.append(speed_update(row['AccX(mg).1'], row['AccY(mg).1'], row['AccZ(mg).1'], ax_list2[-1], ay_list2[-1], az_list2[-1], imu2_positions, DELTA_T, imu2_initial_position))
    ...
    for index, row in imu3.iterrows():
        if not row.isnull().any():
            ...
            speed_prev = speed_list3[-1] if len(speed_list3) > 1 else 0
            speed_update3 = speed_list3.append(speed_update(row['AccX(mg).2'], row['AccY(mg).2'], row['AccZ(mg).2'], ax_list3[-1], ay_list3[-1], az_list2[-1], imu3_positions, DELTA_T, imu3_initial_position))
            ...
            imu3_train_data.append([ax, ay, az, gx, gy, gz, mx, my, mz])
    ...
    # data for train
    train_imu1 = pd.DataFrame(imu1_train_data, columns=['IMU1_AccX', 'IMU1_AccY', 'IMU1_AccZ', 
                                                        'IMU1_GyrX', 'IMU1_GyrY', 'IMU1_GyrZ', 
                                                        'IMU1_MagX', 'IMU1_MagY', 'IMU1_MagZ'])
    train_imu2 = pd.DataFrame(imu2_train_data, columns=['IMU2_AccX', 'IMU2_AccY', 'IMU2_AccZ', 
                                                        'IMU2_GyrX', 'IMU2_GyrY', 'IMU2_GyrZ',
                                                        'IMU2_MagX', 'IMU2_MagY', 'IMU2_MagZ'])
    train_imu3 = pd.DataFrame(imu3_train_data, columns=['IMU3_AccX', 'IMU3_AccY', 'IMU3_AccZ', 
                                                        'IMU3_GyrX', 'IMU3_GyrY', 'IMU3_GyrZ',
                                                        'IMU3_MagX', 'IMU3_MagY', 'IMU3_MagZ'])
